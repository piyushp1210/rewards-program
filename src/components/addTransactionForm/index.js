...
import {Paper, Box, Input, Button, Typography} from '@mui/material';
      ...
      <Paper style={{margin: "0 0 10px 0"}}> 
        <Box p={2} className={"addTransactionFormWrapper"}>
          ...
          <form onSubmit={this.addNewTransaction} className={"addTransactionForm"}>
            ...
            <label>
              Transaction Amount:
              <Input type="number" name="transactionAmount" inputProps={{ min: 0 }} value={this.state.transactionAmount} onChange={this.handleChange} />
            </label>
            <label>
              Transaction Date:
              <Input type="date" name="transactionDateTime" value={this.state.transactionDateTime} onChange={this.handleChange} />
            </label>
            <Button variant="contained" color="primary" type="submit" value="Submit">Add</Button>
            </form>
        </Box>
      </Paper>
    );
